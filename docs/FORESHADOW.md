...
| FS-006 | Never-order guests at 백야 are Circle | `dalbitgol:cafe-baekya` | dialogue: T0 `cafe-hand` hears talk of the missing peddler (no 월향 name); object: stain-free `clean-saucer` (coins the same); after `ember`, saucer/room `when` turns the eye to the café door / 우물길; newspaper: a humor column on "the driest café in the province". 월향=백야 stays unsaid (FS-005 planted only) | Act I (I-8 / I-10) | reinforced |
...
| FS-014 | The market board is already a battlefield | `dalbitgol:market` | object: 관아 notice with a torn corner; newspaper: the error-copy of 한벽일보 sold under it (ties FS-001); after `first_market_sale`, board/room `when` adds 「정거장 화물마당」 pencil / 수레 자국 scribble; dialogue: a stall-keeper who will not look at the board after noon | Act I (I-1 / I-9) | reinforced |
...
| FS-016 | Vanished peddler 강포 was a Circle courier | `dalbitgol:warehouse`; `dalbitgol:warehouse-lane`; `dalbitgol:packing-shed`; `dalbitgol:school` (청람 when); `dalbitgol:market`; `dalbitgol:cafe-baekya` (`cafe-hand`) | object: false-bottom straw and a saucer token in his pack remnant (after_examine bridges to 시세/캐다/만들다); object: wheel ruts / cargo chit concrete only after `first_market_sale`; object: packing-shed `empty-cart` after `ember` points 우물길/다방 문; dialogue: 오씨 / 청람 pack-when nudges 시세, concrete trail when after sale; `cafe-hand` listens to 강포 talk when EmberPrereq; 청람 ember/risk when; newspaper/board `when` after sale; P0/P2 soft-hook has no 월향 NPC | Act I (I-2 / I-8) | reinforced |
