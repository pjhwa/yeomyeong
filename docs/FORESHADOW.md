...
| FS-016 | Vanished peddler 강포 was a Circle courier | `dalbitgol:warehouse`; `dalbitgol:warehouse-lane`; `dalbitgol:packing-shed`; `dalbitgol:school` (청람 when); `dalbitgol:market` | object: false-bottom straw and a saucer token in his pack remnant (after_examine bridges to 시세/캐다/만들다); object: wheel ruts / cargo chit concrete only after `first_market_sale`; dialogue: 오씨 / 청람 pack-when nudges 시세, concrete trail when after sale; newspaper/board `when` after sale; P0 soft-hook has no 월향 NPC | Act I (I-2 / I-8) | reinforced |
