#!/usr/bin/env bash
# First-session 15m loop: hook → pack livelihood nudge → gather/sell → durable clue.
...
if ! (echo >/dev/tcp/"$HOST"/"$PORT") >/dev/null 2>&1; then
  go run ./cmd/server >/tmp/yeomyeong-first-session-15.log 2>&1 &
  started=$!
...
class C:
    ...
    def recv(self):
        try:
            ...
            return False
    def wait(self, needle, timeout=12.0):
...
# warehouse → market → well → well-path → north-street → west-field
time.sleep(1.0)
for step in ["w", "w", "w", "n", "w", "w", "w"]:
    c.send(step)
    time.sleep(0.05)
c.wait("서쪽 밭")
c.send("캐다")
...
time.sleep(1.0)
# west-field → north-street → well-path → well → market
for step in ["e", "e", "e", "s"]:
    c.send(step)
    time.sleep(0.05)
c.wait("달빛골 장터")
