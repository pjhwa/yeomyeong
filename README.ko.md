...
[![Roadmap](https://img.shields.io/badge/milestone-M0-lightgrey)](#로드맵)
...
> **⚠️ 상태: 초기 개발 (M0).** 아래에 적힌 세계는 설계 목표이며 아직 구현된 상태가 아니다.
> 현재 범위는 [로드맵](#로드맵)을 본다.
...
텍스트 MUD가 죽은 이유는 분명하다. 산문 옷을 입은 스프레드시트가 되었기 때문이다.
쥐를 잡아 숫자가 올랐다. 이 형식은 더 나은 대접을 받을 자격이 있다.
...
- **세계가 반응한다.** 시세는 수급·날씨·전황에 따라 움직인다. NPC에게는 일과가 있다.
  플레이어가 대담해지면 서버 전체의 *탄압 수위*가 오르고 순찰이 짙어진다.
- **기억하는 NPC.** 3계층 대화(스크립트 → 오픈웨이트 모델 → 프리미엄 API)로
  주요 인물은 지난 대화를 기억한다. 실제 게임 상태에 접지되고, 서사 진행 전에는
  비밀을 말하지 않는다.

## 배경

1920년대 풍 대체역사. 반도 국가 **달내 (Dalnae)** 는 15년 전 **무쇠 제국**에
국권을 빼앗겼다. 제국은 지맥의 혈점에 쇠말뚝을 박아 땅의 기운 — 그리고 백성의
의지 — 을 수도로 뽑아 간다.

당신은 변방 마을의 아무개로 시작한다. 사라진 연락책, 수상한 조판 오류가 찍힌
신문, 아는 것이 많아 보이는 은퇴한 훈장. 비밀결사 **새벽회** 는 눈치 있는
사람을 찾고 있다.
